...
interface ScaleRequest {
  action: 'scale-up' | 'scale-down' | 'check-preemption';
  bookingId: string;
 ...
 */
export async function vibespecScaler(req: any, res: any) {
  const body: ScaleRequest = req.body;
...
async function scaleUp(booking: any, ref: any) {
  const tier = booking.tier as keyof typeof TIER_CONFIGS;
  ...
  for (const deploy of config.deployments) {
    const replicas = (config.replicas as any)[deploy] || 1;
...
async function scaleDown(booking: any, ref: any) {
  const tier = booking.tier as keyof typeof TIER_CONFIGS;
  const config = TIER_CONFIGS[tier];
  ...
  const costPerHour = booking.vmType === 'spot'
    ? { minimal: 0.70, standard: 3.14, full: 9.66 }[booking.tier as string]
    : { minimal: 2.31, standard: 8.94, full: 23.62 }[booking.tier as string];
...
async function checkPreemption(booking: any, ref: any) {
